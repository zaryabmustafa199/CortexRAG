...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_usage_records_user_month", "usage_records", ["user_id", "month"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            postgresql.ENUM("viewer", "editor", "admin", name="member_role", create_type=False),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "chunk_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("leaf_chunks.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column("workspace_id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column(
            "chunk_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("leaf_chunks.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column("page_number", sa.Integer, nullable=True),
        sa.Column("section_title", sa.String(500), nullable=True),
